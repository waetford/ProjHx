...
###############################################################################
#set variable to match the number of cores for multicore processing. Will provide processing boost for caret function 'train'
...
rf <- randomForest(category ~ ., data = train %>% select(-c(result))) #fit model
...
set.seed(1979, sample.kind = 'Rounding') #make results repeatable
caret_train <- train(category ~., data = train %>% select(-c(result)), method = "rf") #fit model
...
rf_rpart <- rpart(category ~ ., train %>% select(-c(result))) #fit model
...
####ctree
set.seed(1976, sample.kind = 'Rounding') #make results repeatable
t.ctree <- ctree(category ~., data = train %>% select(-c(result))) #fit model
...
#knn
set.seed(1976, sample.kind = 'Rounding') #make results repeatable
c.knn <- train_knn <- train(category ~ ., method = "knn", train %>% select(-c(result))) #fit model
...
##Naive bayes
set.seed(1976, sample.kind = 'Rounding') #make results repeatable
nb <- naive_bayes(category ~ ., data = train %>% select(-c(result)), laplace = 1) #fit model
